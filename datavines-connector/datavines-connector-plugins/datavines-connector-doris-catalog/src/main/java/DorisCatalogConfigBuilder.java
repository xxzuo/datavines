...
@Slf4j
public class DorisCatalogConfigBuilder extends JdbcConfigBuilder {
    ...
    protected InputParam getCatalogInput(boolean isEn) {
        return getInputParam("catalog",
                isEn ? "catalog" : "数据目录",
                isEn ? "please enter catalog" : "请填入数据目录", 1,
                Validate.newBuilder().setRequired(true).setMessage(isEn ? "please enter catalog" : "请填入数据目录").build(),
                null);
    }
